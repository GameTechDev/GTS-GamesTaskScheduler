...
public: // STRUCTORS
        ...
        , m_ringBuffer(nullptr)
    {
        ...
        m_front.store(EMPTY, memory_order::relaxed);
    }
...
public: // ACCESSORS
    ...
    {
        return m_front.load(memory_order::acquire) == EMPTY;
    }
...
public: // MUTATORS
    ...
    GTS_NO_INLINE bool tryPush(Task* pTask)
    {
        ...
        if (f != EMPTY && std::make_signed<size_t>::type(b - f) >= std::make_signed<size_t>::type(pRingBuffer->mask + 1))
        {
        ...
        if (f == EMPTY)
        {
            ...
            m_front.store(b, memory_order::relaxed);
        }
    ...
    GTS_NO_INLINE bool tryPop(Task*& pTask)
    {
        ...
        else
        {
            ...
            if (b == f)
            {
                if (!m_front.compare_exchange_strong(f, EMPTY, memory_order::seq_cst, memory_order::relaxed))
                {
                    pTask = nullptr;
                    result = false;
                }
